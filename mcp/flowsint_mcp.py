"""transgenia-flowsint: stdio MCP wrapper over the flowsint FastAPI.

Governance is DATA, not code: `allowlist.yaml` in this folder is the single
source of truth for what enrichers may be launched. Default-deny is
structural — any name not declared is rejected. See allowlist.yaml.

Modules:
    allowlist_loader   YAML classifier with 60 s mtime TTL
    flowsint_client    HTTP + auth + graph/health helpers (stdlib)
    otel_bootstrap     spans -> OTLP HTTP (feature/mcp-fase0@044c27b, reused)
    metrics            counter -> OTLP + optional Pushgateway (Saurat Q5)
...
Config (.env; see env.example):
    ...
    FLOWSINT_TEMPLATES_DIR    default S:/Transgenia/Flowsint/templates
    OTEL_EXPORTER_OTLP_ENDPOINT   e.g. http://127.0.0.1:4318
    OTEL_SERVICE_NAME             default mcp-flowsint
    PUSHGATEWAY_URL           optional fallback (Saurat Q5 2026-08-20)
"""
import glob
import json
import os
...
TEMPLATES_DIR = ENV.get("FLOWSINT_TEMPLATES_DIR") or "S:/Transgenia/Flowsint/templates"
