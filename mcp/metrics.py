...
Two channels, both opt-in via env:
  1. OTLP metrics via OpenTelemetry SDK -> otel-collector-tgn (127.0.0.1:4318)
     Preferred: the collector re-exports to Prometheus on `otel-collector-tgn:8889`,
     which is already scraped as `job_name: otel-collector` in prometheus.yml.
  2. Prometheus Pushgateway (PUSHGATEWAY_URL) — fallback per Saurat Q5 (2026-08-20)
     if the operator prefers push-based ingestion. Only fires when the env is set.
